...
WHERE `question_id` > 0 OR `question_id` IS NULL;
...
WHERE `ht`.`question_id` > 0 OR `ht`.`question_id` IS NULL;
...
WHERE rc.institution_id IS NULL AND rc.old_funder_id IS NOT NULL;
