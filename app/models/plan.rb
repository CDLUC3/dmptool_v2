class Plan < ActiveRecord::Base
  ...
  scope :reviewed, -> { includes(:plan_states).where(state: ['rejected', 'approved']) }
  ...
  scope :plans_reviewed, ->(institution_id) {joins(:users).where("users.institution_id IN(?)", institution_id).includes(:plan_states).where(state: ['rejected', 'approved']).where(user_plans: {owner: true})}
