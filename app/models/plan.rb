class Plan < ActiveRecord::Base
  ...
  def reviewed
    ...
    self.committed.each do |plan|
      reviewed << plan if self.approved.include?(plan) or self.rejected.include?(plan)
    end
    reviewed
  end
...
scope :plans_reviewed, ->(institution_id) {joins(:users, :current_state).where("users.institution_id IN(?)", institution_id).where(plan_states: {state: 'committed'}).where(user_plans: {owner: true})}
