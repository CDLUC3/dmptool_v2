class Plan < ActiveRecord::Base
  ...
  scope :reviewed, -> { joins(:plan_states).where(state: ['rejected', 'approved']) }
  ...
  scope :plans_reviewed, ->(institution_id) {joins(:users, :plan_states).where("users.institution_id IN(?)", institution_id).where(plan_states: {state: ['rejected', 'approved']}).where(user_plans: {owner: true})}
